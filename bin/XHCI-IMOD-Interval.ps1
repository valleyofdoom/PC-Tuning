...
function main() {
    ...
    foreach ($xhciController in Get-WmiObject Win32_USBController) {
        ...
        if ($verbose) {
            ...
            Write-Host "RTSOFF_value        = capability_address + rtsoff = $($formattedCapabilityAddress) + $(Dec-ToHex -decimal $rtsoff) = $($formattedRTSOFFValue)"
        ...
        for ($i = 0; $i -lt $maxIntrs; $i++) {
            ...
            $interrupterAddress = Dec-ToHex -decimal ([uint64]($runtimeAddress + 0x24 + (0x20 * $i)))
            ...
            if ($verbose) {
                ...
                Write-Host "Write DWORD = $(Dec-ToHex -decimal $desiredInterval)"
            }
